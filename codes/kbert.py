...
class BertClassifier(nn.Module):
    def __init__(self, args, model):
        ...
        print("labels_num:", self.labels_num)
    ...
    def forward(self, src, label, mask, pos=None, vm=None):
        ...
        # print(label.shape)
        # print(logits.shape)
        # loss = self.criterion(self.softmax(logits.view(-1, 2)), label.view(-1, 2))
        # return loss, logits
...
def main():
    ...
    print("args.labels_num:", args.labels_num)
    ...
    def batch_loader(batch_size, input_ids, label_ids, mask_ids, pos_ids, vms):
        ...
        for i in range(instances_num // batch_size):
            ...
            # print("label_ids_batch,line290", label_ids_batch)
        ...
        if instances_num > instances_num // batch_size * batch_size:
            ...
            # print("label_ids_batch,line297", label_ids_batch)
    ...
    def evaluate(args, is_test, metrics='Acc'):
        ...
        if not args.mean_reciprocal_rank:
            ...
            for i, (input_ids_batch, label_ids_batch, mask_ids_batch, pos_ids_batch, vms_batch) in enumerate(
                ...
                input_ids_batch = input_ids_batch.to(device)
                label_ids_batch = label_ids_batch.to(device)
                print("label_ids_batch,line424", label_ids_batch)
                mask_ids_batch = mask_ids_batch.to(device)
                pos_ids_batch = pos_ids_batch.to(device)
                vms_batch = vms_batch.to(device)
